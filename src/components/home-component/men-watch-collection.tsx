...
// import { fourmenproduct } from "@/sanity/lib/queries";
...
//   const products: Product[] = await sanityFetch({ query: fourmenproduct });
...
//           <Link href={`/menwatches/${product.slug.current}`} key={product.slug.current}>
//           <div
...
export default function MenWatchCollection() {
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await sanityFetch({ query: fourmenproduct });
      setProducts(data);
    }
    fetchData();
  }, []);
    ...
    <div>
      ...
      >
        {products.map((product) => (
          ...
          >
            <div className="p-4">
              <Link href={`/menwatches/${product.slug.current}`}>
                <Image
                  src={product.imageUrl}
                  alt={product.productName}
                  width={400}
