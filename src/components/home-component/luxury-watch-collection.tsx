...
// import { fourluxuryproduct} from "@/sanity/lib/queries";
...
// export default async function LuxuryWatchCollection() {
//   const products: Product[] = await sanityFetch({ query: fourluxuryproduct});
...
//           <Link href={`/luxurywatches/${product.slug.current}`} key={product.slug.current}>
//           <div
...
export default function LuxuryWatchCollection() {
  const [products, setProducts] = useState<Product[]>([]);
  const { addToCart } = useCart();

  useEffect(() => {
    async function fetchData() {
      const data = await sanityFetch({ query: fourluxuryproduct });
      setProducts(data);
    }
    fetchData();
  }, []);
    ...
    <div>
      ...
      >
        {products.map((product) => (
          ...
          >
            <div className="p-4">
              <Link href={`/luxurywatches/${product.slug.current}`}>
                <Image
                  src={product.imageUrl}
                  alt={product.productName}
                  width={400}
