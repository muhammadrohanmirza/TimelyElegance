...
// import { allluxuryproduct } from "@/sanity/lib/queries";
...
//   const products: Product[] = await sanityFetch({ query: allluxuryproduct });
...
//           <Link href={`/luxurywatches/${product.slug}`} key={product.slug}>
...
import { sanityFetch } from '@/sanity/lib/fetch';
import { allluxuryproduct } from "@/sanity/lib/queries";
import ProductCardWithAddToCart from '../../components/cart-component/luxuryProductCardWithAddToCart';
...
export default async function MenWatchCollection() {
  const products: Product[] = await sanityFetch({ query: allluxuryproduct });

  return (
    <div>
      <h2 className="text-2xl px-10 py-5 underline font-bold">
        Luxury&apos;s Watches Collection
      </h2>
