...
// import { fourwomenproduct} from "@/sanity/lib/queries";
...
// export default async function WomenWatchCollection() {
//   const products: Product[] = await sanityFetch({ query: fourwomenproduct});
...
//            <Link href={`/womenwatches/${product.slug.current}`} key={product.slug.current}>
//           <div
...
export default function MenWatchCollection() {
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await sanityFetch({ query: fourwomenproduct });
      setProducts(data);
    }
    fetchData();
  }, []);
    ...
    <div>
      ...
      >
        {products.map((product) => (
          ...
          >
            <div className="p-4">
              <Link href={`/womenwatches/${product.slug.current}`}>
                <Image
                  src={product.imageUrl}
                  alt={product.productName}
                  width={400}
