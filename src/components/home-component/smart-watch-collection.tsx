...
// import { foursmartproduct} from "@/sanity/lib/queries";
...
//   const products: Product[] = await sanityFetch({ query: foursmartproduct});
...
//           <Link href={`/smartwatches/${product.slug.current}`} key={product.slug.current}>
//           <div
...
export default function SmartWatchCollection() {
  const [products, setProducts] = useState<Product[]>([]);
  const { addToCart } = useCart();

  useEffect(() => {
    async function fetchData() {
      const data = await sanityFetch({ query: foursmartproduct });
      setProducts(data);
    }
    fetchData();
  }, []);
    ...
    <div>
      ...
      >
        {products.map((product) => (
          ...
          >
            <div className="p-4">
              <Link href={`/smartwatches/${product.slug.current}`}>
                <Image
                  src={product.imageUrl}
                  alt={product.productName}
                  width={400}
